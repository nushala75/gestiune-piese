...
- Actualizarea din `registru-produse-kymco.xlsx`, aflat in radacina aplicatiei, identifica produsele exclusiv prin `Cod - Referinta Prestashop`, previzualizeaza diferentele si nu creeaza produse. Dupa confirmare aplica stocul, pretul final, greutatea, denumirile EN/RO si cantitatea de comandat. Pretul final cu TVA in RON devine `Pret cu TVA` din registru (EUR) inmultit cu cursul EUR/RON editabil, implicit `5.31`, si este rotunjit la 2 zecimale. Stocurile negative si valorile goale din `Nr. produse de comandat` devin zero.
