...
- Actualizarea din `registru-produse-kymco.xlsx`, aflat in radacina aplicatiei, identifica produsele exclusiv prin `Cod - Referinta Prestashop`, previzualizeaza diferentele si nu creeaza produse. Stocul si preturile sunt aplicate impreuna dupa confirmare; pretul final cu TVA in RON devine `Pret cu TVA` din registru (EUR) inmultit cu cursul EUR publicat de BNR si afisat la import, rotunjit la 2 zecimale.
