...
- Actualizarea din `registru-produse-kymco.xlsx`, aflat in radacina aplicatiei, identifica produsele exclusiv prin `Cod - Referinta Prestashop`, previzualizeaza diferentele si nu creeaza automat produse. Dupa confirmare aplica stocul, pretul final, greutatea, denumirile EN/RO si cantitatea de comandat. Pretul final cu TVA in RON devine `Pret cu TVA` din registru (EUR) inmultit cu cursul EUR/RON editabil, implicit `5.31`, si este rotunjit la 2 zecimale. Stocurile negative si valorile goale din `Nr. produse de comandat` devin zero.
- Previzualizarea enumera explicit codurile care corespund mai multor produse si blocheaza aplicarea pana la corectarea lor. Pentru codurile inexistente permite crearea manuala a maximum 10 produse in aceeasi sesiune de import; categoria, unitatea de masura, marca, stocul minim si starea activa sunt confirmate manual, iar codul FGO este alocat automat.
- Salvarile din aplicatie sincronizeaza in registrul Excel randurile existente dupa `cod_produs`: cod, stoc, denumire EN, cantitate de comandat, greutate, pret fara TVA, cota TVA si denumire RO. Pretul fara TVA din Excel este pretul fara TVA RON din aplicatie impartit la ultimul curs EUR/RON memorat. Formula existenta pentru pretul cu TVA este pastrata. Daca registrul este deschis sau nu poate fi scris, tranzactia din baza este anulata si interfata afiseaza valorile nesalvate; produsele absente din registru nu blocheaza salvarea.
